...
Input is a judge directory previously written by
``script/llm_judge/run_justification_judge.py`` (i.e.
``<run-dir>/judge/<output-name>``). This script reads its ``raw/judgement.jsonl``
and writes normalized artifacts into ``<judge-dir>/normalized/``.
"""
...
from src.llm_judge.analysis_utils import should_skip_game_name  # noqa: E402
...
def _norm_key(text: str) -> str:
    text = text.strip()
    # Strip leading "<n>. " / "<n>) " / "<n> - " numeric prefixes.
    text = _NUMERIC_PREFIX_RE.sub("", text)
...
def write_group_csv(
    ...
    with open(path, "w", encoding="utf-8", newline="") as f:
        ...
        for group in sorted(grouped_counts.keys()):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Games to drop before aggregation. Pass --skip-games with no "
            "values to keep all games (default)."
        ),
    )
...
def main() -> None:
    ...
    skip_games = None if args.skip_games is None else tuple(args.skip_games)
    ...
    write_group_csv(
        output_overall_csv,
        grouped_counts={"ALL": normalized_label_counts},
        row_totals={"ALL": rows},
    )
    write_group_csv(
        output_mech_csv,
        grouped_counts=by_mechanism_counts,
        row_totals=dict(mechanism_row_counts),
    )
    write_group_csv(
        output_game_csv,
        grouped_counts=by_game_counts,
        row_totals=dict(game_row_counts),
    )
    write_group_csv(
        output_model_csv,
        grouped_counts=by_model_counts,
        row_totals=dict(model_row_counts),
    )
    ...
    summary = {
        ...
        "mechanism_row_counts": dict(mechanism_row_counts.most_common()),
        "game_row_counts": dict(game_row_counts.most_common()),
        "model_row_counts": dict(model_row_counts.most_common()),
        "mapping_stats": {
            "raw_unique_labels": len(raw_label_counts),
            "normalized_unique_labels": len(normalized_label_counts),
            ...
            "unmapped_raw_label_counts": dict(
                unmapped_raw_counts.most_common()
            ),
        },
        "output_files": {
            ...
            "counts_overall_csv": str(output_overall_csv),
            "counts_by_mechanism_csv": str(output_mech_csv),
            "counts_by_game_csv": str(output_game_csv),
            "counts_by_model_csv": str(output_model_csv),
        },
    }
    ...
    print(f"Top normalized labels: {normalized_label_counts.most_common(10)}")
    print(f"Unmapped raw labels: {dict(unmapped_raw_counts.most_common())}")
