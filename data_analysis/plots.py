...
def plot_mechanism_summary(summary_csv: str | Path, out_dir: str | Path) -> None:
    ...
    aggregated_path = summary_path.with_name("mechanism_summary_aggregated.csv")
    aggregated_rows = _load_csv(aggregated_path) if aggregated_path.exists() else []

    if aggregated_rows:
        rows = aggregated_rows
        coop_key, coop_err_key = "mean_cooperation_rate", "sem_cooperation_rate"
        payoff_key, payoff_err_key = "mean_expected_payoff", "sem_expected_payoff"
        delta_key = "mean_delta_expected_payoff"
    else:
        rows = _load_csv(summary_path)
        coop_key, coop_err_key = "avg_cooperation_rate", None
        payoff_key, payoff_err_key = "avg_expected_payoff", None
        delta_key = "avg_delta_expected_payoff"
    if not rows:
        return
    ...
    for game, items in by_game.items():
        ...
        payoffs = [_safe_float(item.get(payoff_key)) for item in items]
        coop = [_safe_float(item.get(coop_key)) for item in items]
        delta = [_safe_float(item.get(delta_key)) for item in items]
        coop_err = (
            [_nan_to_zero(_safe_float(item.get(coop_err_key))) for item in items]
            if coop_err_key
            else None
        )
        payoff_err = (
            [_nan_to_zero(_safe_float(item.get(payoff_err_key))) for item in items]
            if payoff_err_key
            else None
        )
        sem_sources = (
            sorted({item.get("sem_source") for item in items if item.get("sem_source")})
            if aggregated_rows
            else []
        )
        ...
        bars = ax1.bar(
            ...
            alpha=0.9,
            capsize=4,
            error_kw={"elinewidth": 1.2, "ecolor": COLOR_PALETTE["text"]},
        )
        ax1.bar(
            x + width / 2,
            coop,
        ...
        title = f"Mechanism Summary – {game}"
        if sem_sources:
            title += f"  (error bars = SEM, {'/'.join(sem_sources)})"
        ax1.set_title(title)
