...
class BatchProcessingError(RuntimeError):
    """Raised when one batch fails during parallel processing."""

    def __init__(self, batch_index: int, cause: Exception) -> None:
        super().__init__(f"Batch {batch_index} failed: {cause}")
...
class TextProcessor:
    ...
    def process_text(
        ...
        final_result = {
            ...
            "classification_category_assignments": result.get(
                "category_assignments", {}
            ),
        }
...
class BatchProcessor:
    ...
    def process_texts(
        ...
        Raises:
            BatchProcessingError: If any batch fails while processing texts.
        """
        if not texts:
            return []
        ...
        print(
            f"Processing {len(texts)} texts in {len(batches)} batches with {self.max_workers} workers"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in tqdm(
                ...
                try:
                    ...
                    print(f"Batch {batch_idx} was cancelled: {exc}")
                    raise
                except (
                    TextProcessingError,
                    ...
                    raise BatchProcessingError(batch_idx, exc) from exc
    ...
    def _process_batch(
        self, batch: list[tuple], delay: float = 0.5
    ) -> list[dict[str, Any]]:
        """
        ...
        for text, metadata in batch:
            try:
                ...
                error_index = (
                    None if metadata is None else metadata.get("index")
                )
                raise TextProcessingError(
                    message=(
                        "Failed to process text"
                        if error_index is None
                        else f"Failed to process text at index {error_index}"
                    ),
                    metadata=metadata,
                ) from exc
