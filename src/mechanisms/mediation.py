...
class Mediation(Mechanism):
    ...
    def mediator_mapping(self, mediator: dict[int, Action]) -> Callable:
        ...
        def apply_mediation(
            players_decision: dict[Agent, tuple],
        ) -> dict[Agent, tuple]:
            ...
            for player, decision in players_decision.items():
                action, trace_id = decision[0], decision[1]
                mix_probs = decision[3] if len(decision) > 3 else None
