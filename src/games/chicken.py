...
class ChickenAction(Action):
    """Possible actions in the Chicken game."""

    SAFE = "S"
    DARE = "D"


class Chicken(GridGame):
    """
    Chicken (Hawk-Dove) game environment for two LLM agents.

    Players simultaneously choose between a safe action (A0) and a daring
    action (A1). Mutual daring leads to the worst outcome for both.
    """
