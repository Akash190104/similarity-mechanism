...
class TestInstance(LLM):
    ...
    def invoke(self, prompt: str, **kwargs: Any) -> str:
        if self._is_hle_judge_prompt(prompt):
            return self._fake_hle_judge_response()
        if self._is_hle_prompt(prompt):
            return self._fake_hle_response(prompt)
        if self._is_similarity_judge_prompt(prompt):
            return self._fake_similarity_judge_response()
        ...
        if self._is_multiple_choice_prompt(prompt):
            return self._fake_multiple_choice_response(prompt)
    ...
    def _is_multiple_choice_prompt(self, prompt: str) -> bool:
        """Detect benchmark-style multiple-choice prompts (e.g., Newcomb)."""
        return "FINAL ANSWER:" in prompt

    def _fake_multiple_choice_response(self, prompt: str) -> str:
        """Return a random valid answer for a multiple-choice prompt."""
        # Find all answer options like "A) ...", "B) ...", etc.
    ...
    def _is_similarity_judge_prompt(self, prompt: str) -> bool:
        """Detect subjective similarity / LLM-judge prompts."""
        return "SIMILARITY SCORE:" in prompt

    def _fake_similarity_judge_response(self) -> str:
        """Return a random similarity score for judge prompts."""
        score = random.randint(30, 80)
        return f"Based on my analysis, I see moderate overlap. SIMILARITY SCORE: {score}"

    def _is_hle_prompt(self, prompt: str) -> bool:
        """Detect HLE/GPQA benchmark prompts."""
        return "Explanation: {your explanation" in prompt and "Answer: {" in prompt

    def _fake_hle_response(self, prompt: str) -> str:
        """Return a fake answer for an HLE/GPQA benchmark prompt."""
        # Match only "A) ...", "B) ..." style multiple-choice options
        options = re.findall(r"^([A-Z])\)\s+", prompt, re.MULTILINE)
        if options:
            letter = random.choice(options)
            return f"Explanation: This is a test response.\nAnswer: {letter}"
        # Short answer: return a random placeholder
        return "Explanation: This is a test response.\nAnswer: 42"

    def _is_hle_judge_prompt(self, prompt: str) -> bool:
        """Detect HLE judge prompts."""
        return "Expected answer:" in prompt and "Reply with exactly one word: CORRECT or INCORRECT" in prompt

    def _fake_hle_judge_response(self) -> str:
        """Return a random judge verdict."""
