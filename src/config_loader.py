...
class ConfigLoader:
    ...
    def _is_modular_config(self, config: dict) -> bool:
        ...
        Requires either:
        - All three core gameplay components (game_config, mechanism_config, agents_config)
        - Or similarity elicitation config (game_config, agents_config, similarity)
        """
        full_modular = {'game_config', 'mechanism_config', 'agents_config'}
        similarity_modular = {'game_config', 'agents_config', 'similarity'}

        return (all(key in config for key in full_modular) or
                all(key in config for key in similarity_modular))
    ...
    def _load_modular_config(self, main: dict) -> dict:
        ...
        # Add similarity config (for similarity elicitation)
        if 'similarity' in main:
            resolved['similarity'] = main['similarity']
